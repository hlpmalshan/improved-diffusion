#!/bin/bash
# End-to-end pipeline for CIFAR-100 on improved-diffusion:
...
#
# Update TRAIN_GPUS and SAMPLE_GPUS below to match your hardware.
...
DATA_DIR="/data/diffusion/repos/ddim/ddim_cifar100/datasets/cifar100/train"
LOG_ROOT="/data/diffusion/repos/improved-diffusion/logs_improved_diffusion_cifar100"
...
# CIFAR‑100 hyperparameters (based on your DDIM config)
BATCH_SIZE=128
IMG_SIZE=32
NUM_CH=128
RES_BLOCKS=3
...
echo "==== [Stage 1] Training CIFAR-100 models (reg=0.3, 0.0) on GPUs ${TRAIN_GPUS} ===="

for REG in 0.0 0.3; do
    EXP_DIR="${LOG_ROOT}/cifar100_reg${REG}"
    mkdir -p "${EXP_DIR}"
    export OPENAI_LOGDIR="${EXP_DIR}"
    RES_CKPT=${EXP_DIR}/model406640.pt

    echo "[Train][CIFAR100] reg_val=${REG}"
    CUDA_VISIBLE_DEVICES=${TRAIN_GPUS} \
    torchrun --standalone --nproc_per_node=${TRAIN_NPROC} --master_port=29701 \
...
echo "==== [Stage 2] Sampling from CIFAR-100 models on GPUs ${SAMPLE_GPUS} ===="

# Sampling configuration
NUM_SAMPLES=50000          # generate 50k samples for evaluation
BATCH_SIZE_SAMPLE=512       # per step

# Model/diffusion flags matching training
MODEL_FLAGS="--image_size 32 --num_channels 128 --num_res_blocks 3 --attention_resolutions "16,8" --dropout 0.3 --learn_sigma True"
...
for REG in 0.3 0.0; do
  ...
  CKPT="ema_0.9999_${TOTAL_ITERS}.pt"
  ...
  echo "[Sample][CIFAR100] reg_val=${REG} from ${CKPT_PATH} -> ${SAMPLE_DIR}"
...
echo "==== [Stage 3] Evaluating CIFAR-100 FID / IS / PRDC ===="

OUT_CSV="${LOG_ROOT}/metrics_cifar100.csv"
...
for REG in 0.3 0.0; do
    ...
    if [ ! -d "${SAMPLES_DIR}" ]; then
        echo "[Eval][CIFAR100] Skip reg=${REG}: missing ${SAMPLES_DIR}" >&2
        continue
    fi

    CKPT=$(ls "${EXP_DIR}" | grep '^model' | sort | tail -n1 || true)

    echo "[Eval][CIFAR100] reg_val=${REG}"
    ...
    echo "cifar100,${REG},${EXP_DIR},${CKPT},${SAMPLES_DIR},${FID},${IS_MEAN},${IS_STD},${PREC},${REC},${DENS},${COV}" >> "${OUT_CSV}"
done

echo "==== Done (train → sample → eval for CIFAR-100) ===="
